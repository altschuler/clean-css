...
module.exports = function Comments(keepSpecialComments) {
  ...
  return {
    ...
    stripSpaces: function(data) {
      return data.replace(new RegExp(comments.placeholderPattern + ' ', 'g'), '$1');
    },

    restore: function(data) {
      var restored = 0;

      return data.replace(comments.placeholderRegExp, function(match, placeholder) {
        restored++;

        switch (keepSpecialComments) {
          case '*':
            return comments.restore(placeholder);
          case 1:
            return restored == 1 ?
              comments.restore(placeholder) :
              '';
          case 0:
            return '';
        }
